...
//init of externalInterruption
    ...
    */
    ...
    wd('0');
    */
    unsigned int jiShiInt[5];
    unsigned int jiShiFloat[5];
    char cursor=0;
     char cur=0;
...
*/  unsigned char lo;
        ...
        for(lo=0;lo<4;lo++){
            if((0xc0+lo*7+cursor+5-30>=0xc0)&&(0xc0+lo*7+cursor+5-30<0xd0))
                wc(0xc0+(lo)*7+cursor+5-30);
                wd((((jiShiFloat[lo])*100)/32768)%10+'0');

            if((0xc0+lo*7+cursor+4-30>=0xc0)&&(0xc0+lo*7+cursor+4-30<0xd0))
                wc(0xc0+(lo)*7+cursor+4-30);
                wd(((jiShiFloat[lo])*10)/32768+'0');
            if((0xc0+lo*7+cursor+3-30>=0xc0)&&(0xc0+lo*7+cursor+3-30<0xd0))
                wc(0xc0+lo*7+cursor+3-30);
                wd('.');
            if((0xc0+lo*7+cursor+2-30>=0xc0)&&(0xc0+lo*7+cursor+2-30<0xd0))
                wc(0xc0+lo*7+cursor+2-30);
                wd(jiShiInt[lo]%10+'0');
            if((0xc0+lo*7+cursor+1-30>=0xc0)&&(0xc0+lo*7+cursor+1-30<0xd0))
                wc(0xc0+lo*7+cursor+1-30);
                wd((jiShiInt[lo]%100)/10);
            if((0xc0+lo*7+cursor-30>=0xc0)&&(0xc0+lo*7+cursor-30<0xd0))
                wc(0xc0+lo*7+cursor-30);
                wd((jiShiInt[lo]/100));
        }
        cur++;
        cursor+=(cur%50);
        ...
        if(ifci==1){
            ...
            //tmp_Time为整数
            //record_TAR为小数
            jiShiInt[ci]=tmp_Time;
            jiShiFloat[ci]=record_TAR;
